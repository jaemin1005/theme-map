...
pub fn init(cfg: &mut web::ServiceConfig) {
    cfg.service(
        web::scope("")
            ...
            .route("/map_me/likes", web::get().to(search::search_me_like))
            .route("/map_read", web::post().to(map::map_read))
            .route("/map_remove", web::post().to(map::map_remove))
            .route("/search", web::get().to(search::search))
            .route("/maps/{map_id}/like", web::post().to(map::map_like)) 
            .route("/maps/{map_id}/like", web::delete().to(map::map_dislike)), 
    )
